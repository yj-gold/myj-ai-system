"""
Modèle de relevé MYJ Capital — VERSION 1 (gelée le 2026-08-22).

  ⚠️  FICHIER ARCHIVÉ — NE JAMAIS MODIFIER  ⚠️

Ce fichier est le modèle exact utilisé pour tous les relevés déjà envoyés aux
clients. Il est verrouillé par un hash SHA-256 enregistré dans
`templates/LOCK.json` et vérifié par `verify_templates.py` (bloquant en CI).

Toute évolution du design ou du contenu se fait en créant une NOUVELLE version
(`releve_v2.py`, `releve_v3.py`, …), jamais en éditant celle-ci : c'est ce qui
garantit qu'un relevé de 2026-03 pourra toujours être régénéré à l'identique.

Identité visuelle MYJ Capital :
  - Bandeau : teal profond (#1B4D47), wordmark MYJ | CAPITAL blanc
  - Corps   : quasi-noir (#0b1a18), cartes teal
  - Accents : teal (#2ab5a5)
  - P&L positif : #27ae60  |  négatif : #e74c3c
"""
...
def render(
    ...
    """Interface uniforme appelée par le moteur de relevés.

    La v1 ignore `client` et `history` (le modèle d'origine ne les affichait
    pas) — ces paramètres existent pour que toutes les versions partagent la
    même signature d'appel.
    """
